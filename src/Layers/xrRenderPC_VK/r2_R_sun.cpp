...
void render_sun_old::render_sun()
{
    ...
    if (_abs(m_fCosGamma) < 0.99f && ps_r2_ls_flags.test(R2FLAG_SUN_TSM))
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            frustumPnts[i] = eyeFrustum.pntList[(i << 1)];
            frustumPnts[i + 4] = eyeFrustum.pntList[i << 1 | 0x1];
        }

        glm::vec3 leftVector, viewVector;
        const glm::vec3 eyeVector(0.f, 0.f, -1.f);

        glm::mat4 translate = glm::translate(m_View, m_lightDir);
        ...
        float delta_proj = m_fTSM_Delta * lambda;
        const float xi = -0.6f;
    ...
    if (ps_r2_ls_flags.test(R2FLAG_SUN_FOCUS))
    {
        ...
        for (const auto& corner : sun::corners)
        {
            pt = wform(x_full_inverse, corner);
            pt = wform(xform, pt);
    ...
    // Render shadow-map
    {
        ...
        if (bNormal || bSpecial)
        {
            ...
            if (bSpecial)
            {
                ...
                dsgraph.render_graph(1);
                dsgraph.render_sorted();
            }
        }
    }
